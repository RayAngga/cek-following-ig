...
function parseCSV(t){
  ...
  let col=0;
  const unameIdx=first.findIndex(h=>/^username$/i.test(h));
  if(unameIdx>=0) col=unameIdx;
  const start=unameIdx>=0?1:0, out=[];
  for(let i=start;i<lines.length;i++){
    const cols=lines[i].split(',').map(s=>s.trim().replace(/^"|"$/g,''));
    const u=(cols[col]||'').replace(/^@/,'').toLowerCase();
    if(u) out.push(u);
  }
...
function extractUsernamesFromFollowers(d){
  // format baru: { relationships_followers: [...] }
  if(d && Array.isArray(d.relationships_followers)){
    return d.relationships_followers
      .map(it=>it?.string_list_data?.[0]?.value)
      .filter(Boolean)
      .map(s=>s.toLowerCase());
  }
  ...
  if(Array.isArray(d)){
    ...
    const b=d
      .map(it=>it?.username||it?.name||it?.value)
      .filter(Boolean)
...
function extractUsernamesFromFollowing(d){
  // Format baru: { relationships_following: [ { title, string_list_data } ] }
  if (d && Array.isArray(d.relationships_following)) {
    return d.relationships_following
      .map(it => {
        if (it?.title) return it.title; // username utama
        if (it?.string_list_data?.[0]?.value) return it.string_list_data[0].value;
        const href = it?.string_list_data?.[0]?.href;
        if (href) {
  ...
  if (Array.isArray(d)) {
    const arr = d
      .map(it => {
        ...
        return it?.username || it?.name || it?.value || null;
      })
      .filter(Boolean)
      .map(s => String(s).toLowerCase());
...
async function parseFile(file, kind){
  ...
  try{
    ...
    return kind==='followers'
      ? extractUsernamesFromFollowers(json)
      : extractUsernamesFromFollowing(json);
  }catch(e){
    throw new Error(`File ${file.name} tidak valid (${ext.toUpperCase()}).`);
  }
}

/* ===== ZIP Parsing ===== */

async function parseZipInstagram(file){
  if(typeof JSZip === 'undefined'){
    throw new Error('JSZip tidak ditemukan. Pastikan JSZip sudah di-load.');
  }
  const zip = await JSZip.loadAsync(file);

  let followersPath = null;
  let followingPath = null;

  zip.forEach((path, f) => {
    if (path.includes('followers_and_following/followers_1.json')) {
      followersPath = path;
    }
    if (path.includes('followers_and_following/following.json')) {
      followingPath = path;
    }
  });

  if (!followersPath || !followingPath) {
    throw new Error('ZIP tidak berisi followers_1.json / following.json di folder followers_and_following.');
  }
  ...
  const followers = extractUsernamesFromFollowers(followersJson);
  const following = extractUsernamesFromFollowing(followingJson);

  return { followers, following };
}

/* ===== Dropzones (followers & following) ===== */
$$('.dropzone').forEach(zone=>{
  const type=zone.dataset.type;
  const input = type === 'followers' ? followersInput
               : type === 'following' ? followingInput
               : null;
  const label = type === 'followers' ? followersFileName
               : type === 'following' ? followingFileName
               : null;
...
followersInput?.addEventListener('change',()=>{
  followersFileName.textContent=followersInput.files[0]?followersInput.files[0].name:'Belum ada file';
});
followingInput?.addEventListener('change',()=>{
  followingFileName.textContent=followingInput.files[0]?followingInput.files[0].name:'Belum ada file';
});

zipInput?.addEventListener('change', ()=>{
  const file = zipInput.files[0];
  zipFileName.textContent = file ? file.name : 'Belum ada file ZIP';
  if(file){
...
function makeRow(u){
  ...
  return `
    <div class="flex items-center justify-between px-4 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/80 transition">
      <span class="username font-medium">@${u}</span>
      <a href="${url}" target="_blank" rel="noopener noreferrer" class="text-sm font-semibold text-violet-700 dark:text-violet-300 hover:text-violet-900 dark:hover:text-violet-100">Profil →</a>
    </div>`;
}
...
function renderLists(){
  ...
  const list1=(q1?state.notFollowingBack.filter(u=>u.includes(q1)):state.notFollowingBack)
    .map(makeRow).join('')||`<p class="px-6 pb-4 text-sm text-gray-500 dark:text-slate-400">Tidak ada data yang cocok.</p>`;
  const list2=(q2?state.fans.filter(u=>u.includes(q2)):state.fans)
    .map(makeRow).join('')||`<p class="px-6 pb-4 text-sm text-gray-500 dark:text-slate-400">Tidak ada data yang cocok.</p>`;

  notFollowingBackList.innerHTML=list1;
  fansList.innerHTML=list2;
}

/* ===== Core processing ===== */

function applyFollowersFollowing(followers, following){
...
/* ===== Actions: Manual followers + following ===== */
processBtn.addEventListener('click', async ()=>{
  if(!followersInput.files[0] || !followingInput.files[0]){
    alert('Harap unggah kedua file (followers dan following), atau gunakan upload ZIP.');
    return;
  }
  try{
    processBtn.disabled=true;
    setStatus('Memproses data…','info');

    const [followers, following]=await Promise.all([
      parseFile(followersInput.files[0],'followers'),
      parseFile(followingInput.files[0],'following')
    ]);

    applyFollowersFollowing(followers, following);
    setStatus('Selesai ✔','ok');
  }catch(err){
    console.error(err);
    alert(`Terjadi kesalahan: ${err.message}`);
    setStatus('Gagal memproses. Cek format file Anda.','err');
  }finally{
...
async function handleZipProcess(file){
  try{
    setStatus('Memproses ZIP Instagram…','info');
    const { followers, following } = await parseZipInstagram(file);
    if(!followers.length || !following.length){
      setStatus('ZIP terbaca, tapi followers/following kosong. Cek lagi data ekspor Anda.','warn');
    }
    applyFollowersFollowing(followers, following);
    setStatus('Selesai dari ZIP ✔','ok');
  }catch(err){
    console.error(err);
    alert(`Gagal membaca ZIP: ${err.message}`);
    setStatus('Gagal memproses ZIP. Pastikan ZIP asli dari Instagram.','err');
  }
}
...
btnClear.addEventListener('click', ()=>{
  ...
  followersFileName.textContent='Belum ada file';
  followingFileName.textContent='Belum ada file';
  if(zipInput){
    zipInput.value='';
    if(zipFileName) zipFileName.textContent='Belum ada file ZIP';
  }
...
btnToggle.addEventListener('click', async ()=>{
  if(showingUpload){
    ...
    btnToggle.textContent='Kembali ke Unggah Data';
    btnToggle.setAttribute('aria-expanded','true');
    instruksiSection.setAttribute('aria-hidden','false');
    await animateSwap(uploadSection, instruksiSection);
    btnToggle.disabled=false;
  }else{
    btnToggle.disabled=true;
    btnToggle.textContent='Cara Unduh Data';
    btnToggle.setAttribute('aria-expanded','false');
    instruksiSection.setAttribute('aria-hidden','true');
    await animateSwap(instruksiSection, uploadSection);
    btnToggle.disabled=false;
  }
  showingUpload=!showingUpload;
  (showingUpload?uploadSection:instruksiSection)
...
function showLang(lang){
  ...
  toShow.classList.remove('lang-enter');
  void toShow.offsetWidth;
  toShow.classList.add('lang-enter');
  currentLang = lang;
}
